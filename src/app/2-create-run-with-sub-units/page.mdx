...
Sometimes, units under test (UUT) include sub-units. They can be specified when creating runs, automatically linking them as children to the main unit for full traceability throughout manufacturing.

## Example with single sub-unit

Here's how to create a test run with sub-units:

```python {{ title: 'sub_unit_upload.py'}}
from tofupilot import TofuPilotClient
import time

client = TofuPilotClient(api_key="Your API key here")
