...
```bash {{ title: 'cURL' }}
curl -X POST https://www.tofupilot.com/api/v1/runs -H "Content-Type: application/json" -H "Authorization: Bearer ${TOFUPILOT_API_KEY}" -d '{
    "procedure_id": "FVT1",
    "unit_under_test": {
      "serial_number": "PCBA01-0001",
      "part_number": "PCB01"
...
```bash {{ title: 'cURL' }}
curl -X POST https://www.tofupilot.com/api/v1/import -H "Content-Type: application/json" -H "Authorization: Bearer ${TOFUPILOT_API_KEY}" -d '{
    "file_path": "path/to/your/test/report.json",
    "importer": "OPENHTF"
}'
```
...
```bash {{ title: 'cURL' }}
curl -X GET "https://www.tofupilot.com/api/v1/runs?serial_number=PCBA01-0001" -H "Authorization: Bearer ${TOFUPILOT_API_KEY}"
```

```js
fetch('https://www.tofupilot.com/api/v1/runs?serial_number=PCBA01-0001', {
  method: 'GET',
  headers: {
    Authorization: `Bearer ${process.env.TOFUPILOT_API_KEY}`,
  },
...
## Update unit {{ tag: 'PATCH', label: '/v1/units' }}

<Row>
  <Col>
    This endpoint allows you to update a specific unit.
    <Properties>
      <Property name="serial_number" type="string, required">
        The unique identifier of the given unit. _Example: "PCBA01-001"_
      </Property>
      <Property name="sub_units" type="array(record), optional">
        Identifiers of [sub-units](/sub-units).
      </Property>
  ...
  <Col sticky>
    <CodeGroup title="Request" tag="PATCH" label="/v1/units">
...
```bash {{ title: 'cURL' }}
curl -X PATCH https://www.tofupilot.com/api/v1/units -H "Content-Type: application/json" -H "Authorization: Bearer ${TOFUPILOT_API_KEY}" -d '{
    "serial_number": "PCBA01-0001",
    "sub_units": [
        {"serial_number": "CELL01-0001"},
        {"serial_number": "CELL01-0002"},
...
```js
fetch('https://www.tofupilot.com/api/v1/units', {
  method: 'PATCH',
  headers: {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${process.env.TOFUPILOT_API_KEY}`,
