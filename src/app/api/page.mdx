...
```bash {{ title: 'cURL' }}
curl -X PATCH https://www.tofupilot.app/api/v1/units -H "Content-Type: application/json" -H "Authorization: Bearer ${TOFUPILOT_API_KEY}" -d '{
    "serial_number": "PCBA01-0001",
    "sub_units": [
        {"serial_number": "CELL01-0001"},
        {"serial_number": "CELL01-0002"},
...
```js
fetch('https://www.tofupilot.app/api/v1/units', {
  method: 'PATCH',
  headers: {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${process.env.TOFUPILOT_API_KEY}`,
