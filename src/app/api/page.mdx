...
```bash {{ title: 'cURL' }}
curl -X POST https://www.tofupilot.com/api/v1/runs?serial_number=<serial_number> -H "Content-Type: application/json" -H "Authorization: Bearer ${TOFUPILOT_API_KEY}"
```

```js
fetch('https://www.tofupilot.com/api/v1/runs?serial_number=<serial_number>', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${process.env.TOFUPILOT_API_KEY}`,
