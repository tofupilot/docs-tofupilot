export const metadata = {
  title: 'Manufacturing Test Traceability & Analytics | OpenHTF',
}

# OpenHTF

Integrate your OpenHTF scripts with TofuPilot in a single line. {{ className: 'lead' }}

<Image
  src="/openhtf-header.png"
  alt=""
/>


## Overview

[OpenHTF](https://openhtf.com) is an open-source Python framework for automating hardware tests. It makes writing and running tests easier with a modular design. Built at Google, it's now used by teams around the world.

By <u>adding a single line</u> to your OpenHTF script, TofuPilot's open-source Python client automatically creates a run on TofuPilot, capturing the test phases, measurements, and attachments, and storing the original OpenHTF JSON log for traceability.

## Installation

1. [Create a TofuPilot account](https://docs.tofupilot.com/user-management#sign-up)

2. Install OpenHTF:

```sh
pip install openhtf six setuptools
```

3. Install the TofuPilot open-source Python client:

```sh
pip install tofupilot
```

Add it to your test like this:

```python {{ title: 'main.py' }}
import openhtf as htf
from tofupilot.openhtf import TofuPilot # Import OpenHTF for TofuPilot

def phase_one(test):
    return htf.PhaseResult.CONTINUE

def main():
    test = htf.Test(phase_one)
    with TofuPilot(test):               # One-line integration
      test.execute(lambda: "PCB001")

if __name__ == '__main__':
    main()
```

Your OpenHTF reports are uploaded to your TofuPilot account using the Unit Under Test serial number `dut_id` (e.g., "PCB001"), which you can find on the **Runs** page.

<Image
  src="/output-callbacks-page-run.png"
  alt="Run page showing the different steps performed for a Unit Under Test (UUT) in OpenHTF with TofuPilot."
/>

## Supported features

We are continuously adding features to the integration. Below is a table of OpenHTF features currently supported by TofuPilot.

<table>
  <thead>
    <tr>
      <th>OpenHTF features</th>
      <th></th>
      <th>TofuPilot support</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>**Phases**</td>
      <td>Phases</td>
      <td>✅</td>
    </tr>
    <tr>
      <td />
      <td>Sub-phases</td>
      <td>✅</td>
    </tr>
    {/* <tr>
      <td />
      <td>Skipped phases</td>
      <td>⌛</td>
    </tr> */}
    {/* <tr>
      <td />
      <td>Logger</td>
      <td>⌛</td>
    </tr> */}
    {/* <tr>
      <td />
      <td>Diagnosis</td>
      <td>⌛</td>
    </tr> */}
    {/* <tr>
      <td />
      <td>Plugs</td>
      <td>⌛</td>
    </tr> */}
    <tr>
      <td>**Measurements**</td>
      <td>Numeric measurement</td>
      <td>✅</td>
    </tr>
    <tr>
      <td />
      <td>Boolean measurement</td>
      <td>✅</td>
    </tr>
    <tr>
      <td />
      <td>String measurement</td>
      <td>✅</td>
    </tr>
    <tr>
      <td />
      <td>Limits</td>
      <td>✅</td>
    </tr>
    <tr>
      <td />
      <td>Units</td>
      <td>✅</td>
    </tr>
    {/* <tr>
      <td />
      <td>Marginal limits</td>
      <td>⌛</td>
    </tr> */}
    {/* <tr>
      <td />
      <td>Dimensions</td>
      <td>⌛</td>
    </tr> */}
    {/* <tr>
      <td />
      <td>Multiple measurements</td>
      <td>⌛</td>
    </tr> */}
    <tr>
      <td>**Attachments**</td>
      <td>File</td>
      <td>✅</td>
    </tr>
    <tr>
      <td />
      <td>File from path</td>
      <td>✅</td>
    </tr>
    {/* <tr>
      <td />
      <td>Multiple measurements</td>
      <td>⌛</td>
    </tr> */}
    <tr>
      <td>**Output Callback**</td>
      <td>JSON Report</td>
      <td>✅</td>
    </tr>
  </tbody>
</table>

## More documentation

Discover TofuPilot integrations in [Test Runs](/procedures) sections and OpenHTF features in our dedicated documentation maintained by our team.

<Button variant="text" arrow="right" href="https://openhtf.org">
  <>OpenHTF Documentation</>
</Button>
