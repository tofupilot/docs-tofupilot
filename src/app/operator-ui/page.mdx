...
2. A unique URL will appear in your console.
   <Code language="bash" code={`2025-05-02 17:45:29 - ✅ Streaming: Interactive stream successfully started at: https://www.tofupilot.app/[your_organization]/streaming/[uuid]`} />
3. Click the link to open the Operator UI in TofuPilot.

<Note>
  Anyone with the URL can view the test run in progress. Use either a user API key or a station API key. With a user API key, only the named user can submit prompts during the test.
</Note>
...
The Operator UI updates in real time as the test progresses. In this screenshot, the test has completed and is now waiting for the next execution to begin.
...
Even before the test is started, you can already see the associated metadata: procedure ID, version, part number, and hardware revision.

### Custom Operator Prompts

You can configure custom prompts in your test script. In this example, the operator is asked to confirm the LED color. This allows you to combine automated steps with manual validation.
...
Each test phase is displayed as it runs, along with its duration and result. You can track all measurement types in real time — numeric, string, boolean, or even tables — along with their units, limits, and validation status.
