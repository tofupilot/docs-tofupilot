...
<CodeGroup label="create_run.py">

```python {{ title: 'Pytest plugin' }}
import time
from tofupilot import conf

# Setting test metadata with procedure ID, serial number, and part number
conf.set(procedure_id="FVT1", serial_number="PCBA01-0001", part_number="PCBA01")


def test_simple():
    # Simulating a test run with a 1-second duration
    # The duration will be automatically measured and recorded in the test run
    time.sleep(1)

    # If all assertions pass, the test will be marked with a Pass status
    assert True
```
...
response = client.create_run(
    ...
    run_passed=True, # Status of the test run
    duration=timedelta(minutes=12, seconds=13) # Test duration
)
```
...
```bash {{ title: 'Pytest plugin' }}
pytest --tofupilot create_run.py
```

```bash {{ title: 'Python client' }}
