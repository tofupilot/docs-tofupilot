...
<Note>
  If a procedure has `steps` but the procedure duration is not defined, the `duration` will be automatically calculated as the sum of the durations of its steps
</Note>
...
These features will be explained in more detail in the [Test steps analysis](/analysis) section.
