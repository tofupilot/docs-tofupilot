...
### Interactif prompt desactivated

Instead of using the basic run installer, you need to:

1. Generate a template configuration files:
...
2. Open, fill-up, and save the configuration file:
...
3. Install or update image by charging configuration files:
...
4. You can check the installation log with:
...
If you're using a self-signed certificate, make sure to register its path during setup. To confirm it's working, open your app in a browser and verify that the certificate is valid and HTTPS loads correctly.

In your Python script, pass the path to your certificate using the verify parameter: `verify="/path/to/selfsigned.crt"`
...
def main():
    client = TofuPilotClient(
        url="<App Domain>",
        verify="/path/to/selfsigned.crt", # Set the path to your certificates
      )
