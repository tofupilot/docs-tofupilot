...
<Note>
  If a run has `steps` but no defined `duration`, the `duration` will be
  automatically calculated as the sum of the durations of its steps (e.g., 2
  minutes and 30 seconds in this example).
</Note>
...
<Properties>
  ...
  <Property name="duration" type="string, required">
    ...
    - For Python Client: Use a [`timedelta`](https://docs.python.org/3/library/datetime.html#timedelta-objects) object. _Example: `timedelta(hours=1, minutes=23, seconds=12)`_
